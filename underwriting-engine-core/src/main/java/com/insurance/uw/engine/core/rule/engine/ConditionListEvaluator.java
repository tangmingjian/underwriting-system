...
/**
 * 条件列表评估器 — 支持 AND/OR 嵌套逻辑和 12 种内置比较运算符。
 *
 * <p>通过 {@link OperatorHandler} SPI 可扩展自定义操作符或覆盖内置逻辑。</p>
 */
public class ConditionListEvaluator implements RuleEvaluator {
    ...
    /**
     * 使用内置操作符 + 自定义处理器。
     * 自定义处理器优先于内置逻辑，多个处理器按 list 顺序匹配。
     */
    public ConditionListEvaluator(ObjectMapper objectMapper, List<OperatorHandler> operatorHandlers) {
        this.objectMapper = objectMapper;
        this.operatorHandlers = operatorHandlers != null ? operatorHandlers : List.of();
    }
    ...
    private boolean compare(Object featureValue, String operator, Object configValue) {
        ...
        for (OperatorHandler handler : operatorHandlers) {
            if (handler.supports(operator)) {
                return handler.compare(featureValue, operator, configValue);
            }
        }
